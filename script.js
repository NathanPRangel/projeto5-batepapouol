...
enter_mensagem.addEventListener("keydown", function (e) {
    if (e.key === "Enter") {
        e.preventDefault();
        enviarMensagem();
    }
...
enter_login.addEventListener("keydown", function (e) {
    if (e.key === "Enter") {
        e.preventDefault();
        logar();
    }
...
function perguntarNome() {
    ...
    if (nomeUsuario.name) {
        document.querySelector(
            ".avisoNome"
        ).innerHTML = `O nome ${nomeUsuario.name} já está cadastrado, tente outro nome.`;
    } else {
        document.querySelector(
            ".avisoNome"
        ).innerHTML = `Por favor digite um nome válido.`;
    }
}

function requisitarMensagens() {
    const promessaMensagens = axios.get(
...
function rotularMensagens(objeto) {
    ...
    if (objeto.type === "status") {
        ...
        objeto.type === "private_message" &&
        (objeto.to === nomeUsuario.name || objeto.from === nomeUsuario.name)
    ) {
        classe = "private";
...
function permanencia() {
    const requisicaoStatus = axios.post(
...
function enviarMensagem() {
    ...
    let requisicaoMensagem = axios.post(
        "https://mock-api.driven.com.br/api/vm/uol/messages",
        objetoMensagem
    );
...
function requisitarUsuarios() {
    const promessaUsuarios = axios.get(
        "https://mock-api.driven.com.br/api/vm/uol/participants"
    );
...
function trocarAutomatico() {
    if (contato === "") {
        ...
        document.querySelector(
            ".descricaoMensagem"
        ).innerHTML = `Enviando para Todos`;
        visibilidade = "Público";
    } else {
        document.querySelector(".reservado").classList.remove("desabilitado");
        document.querySelector(
            ".descricaoMensagem"
        ).innerHTML = `Enviando para ${contato} ${visibilidade === "Público" ? "(publicamente)" : "(reservadamente)"
        }`;
    }
}
...
function selecionarContato(elemento) {
    ...
    document
        .querySelector(".listaUsuarios .selecionado")
